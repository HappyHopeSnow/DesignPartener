public class Main {
    ...
    public static void main(String[] args) {
         ...
         *
         ****************结构型分割线******************************************************
         *       12.适配器模式：对于已经存在的接口，无法满足现状，需要创建新的接口(需要通用的定义出来)，并继承原有的实现类，这样，新的实现类就有了原来的类的对应方法，来满足现状。
         *              优点：利用继承特性。
         *              缺点：无法动态扩展
         *       13.装饰器模式：利用构造函数入参数，持有实例。
         *              优点：方便动态扩展
         *              缺点：
         *       14.桥接模式：抽象类持有接口，对于接口的不同实例调用.接口定义通用方法，不同实例去实现，抽象桥对象持有接口，并获取get和set方法。
         *       15.代理模式：对于类的代理扩展。
         *       16.外观模式：抽象出一个整体类，来持有和管理对应的对象，这样子，降低对象间的耦合。
         *       17.组合模式：定义一种访问集合和单体相同的方式。
         *              优点：
         *              缺点：
         *       18.享元模式：数据库连接池。将一些公共的对象，属性，添加到集合中复用。
         *
         *
         *****************创建型分割线******************************************************
         *        19.工厂模式： 通过一个参数，来界定返回对象类型。
         *        20.抽象工厂：
         *        21.单例模式：
         *        22.原型模式：
         *        23.建造者：
